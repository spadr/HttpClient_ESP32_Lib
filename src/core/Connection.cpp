...
namespace canaspad
{
    ...
    bool WiFiSecureConnection::isConnectionValid(const std::string &host, int port) const
    {
        ...
        return (now - m_lastUsed <= m_keepAliveTimeout) &&
    ...
    bool WiFiSecureConnection::connect(const std::string &host, int port)
    {
        ...
        while (!result)
        {
            ...
            result = WiFiClientSecure::connect(host.c_str(), port); // WiFiClientSecure::connect を呼び出す
            if (!result)
            {
                Serial.printf("Connection failed. Error code: %d\n", getLastError()); // getLastError() を呼び出す
                // 短い遅延を入れて再試行
                delay(100);
            }
        }
    ...
    {
        setTimeout(m_writeTimeout.count() / 1000); // WiFiSecureConnection::setTimeout() を呼び出す
        return WiFiClientSecure::write(buf, size);
    }

    int WiFiSecureConnection::read(uint8_t *buf, size_t size)
    {
        setTimeout(m_readTimeout.count() / 1000); // WiFiSecureConnection::setTimeout() を呼び出す
    ...
    {
        // ミリ秒から秒に変換（切り上げ）
        m_connectTimeout = std::chrono::duration_cast<std::chrono::seconds>(connectTimeout + std::chrono::milliseconds(999)) / 1000;
        m_readTimeout = std::chrono::duration_cast<std::chrono::seconds>(readTimeout + std::chrono::milliseconds(999)) / 1000;
        m_writeTimeout = std::chrono::duration_cast<std::chrono::seconds>(writeTimeout + std::chrono::milliseconds(999)) / 1000;

        // WiFiClientSecure のタイムアウトを設定
        WiFiClientSecure::setTimeout(std::max({m_connectTimeout, m_readTimeout, m_writeTimeout}).count());
    }
    ...
    {
        return const_cast<WiFiClientSecure *>(static_cast<const WiFiClientSecure *>(this))->connected();
    }
